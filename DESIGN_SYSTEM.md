...
- **プライマリーは黒(`--sk-accent: #111418`)。** 有彩色を持たない完全モノクローム系。インタラクティブ要素は色相ではなく形(黒塗りピル・塗りチップ)で区別する。既定は不変で、ブランド色を 1 色だけ乗せたい場合のみ opt-in で切替できる(§プライマリー色の切替)。
...
**既定はモノクローム(プライマリー=黒 → ダークで近白)で不変。** ブランドの識別色を 1 色だけ乗せたい場合に限り opt-in で切り替えられる。憲法は破らない — 差し替わるのは**すでに不透明が許された accent 面(`.sk-btn--primary` / `.sk-chip--accent` / `.sk-badge`)と、フォーム選択状態(switch/check/radio)・進捗・フォーカスリングの「色相」だけ**。ガラス面(`.sk-glass`)には一切色を塗らない。意味色(`--sk-danger` / `--sk-ok`)も不変。

- **プリセット**: ルート(`<html>` か `<body class="sk">`)に `.sk-accent-<hue>` を付ける。用意した色は blue / teal / green / purple / pink / orange の6種。
- **任意色**: `--sk-accent` と、その面上の文字/チェック色 `--sk-on-accent` を直接指定する。`--sk-on-accent` は accent とコントラストが取れる白か黒を選ぶ(showcase では明度から WCAG コントラスト比で自動判定)。
- **テーマ反転**: モノクロと同じ流儀で light/dark に反転する。light=中濃色+白文字、dark=一段明色+暗文字。これで「accent を前景(quiet ボタン・フォーカス)に使っても暗下地で沈まない」ことと「accent 面上の文字が読める」ことを両テーマで両立する。`.sk-dark` / `.sk-auto` との併用で自動的に切り替わる。

```html
<html class="sk-accent-blue">                          <!-- プリセット -->
<html style="--sk-accent:#7A5AF8; --sk-on-accent:#fff">  <!-- 任意色 -->
<html class="sk-dark sk-accent-green">                  <!-- ダーク併用(自動で明色へ反転) -->
```

> SwiftUI 版も同機構を提供する: `.sukashiAccent(.blue)` 等のプリセット/任意色(`Sources/Sukashi/SukashiAccent.swift`)。
...
**憲法を保ったまま、操作の手応えだけを足す。** すべて transform / opacity / 影のみで表現し、色は増やさない。`prefers-reduced-motion` では §アクセシビリティの全モーション停止によって動きが消え、最終状態(機能)だけが残る。hover は実ポインタ環境(`@media (hover: hover) and (pointer: fine)`)に限定し、タッチでの hover 固着を避ける。

- **反応する材質**: `.sk-btn` / `.sk-iconbtn` は hover で 1px 浮き(`translateY(-1px)`)、press で沈む(既存の scale)。既存の transform 遷移に乗るだけで新トークンは足さない。
- **非選択セグ/タブの hover**: `.sk-seg__item` / `.sk-tabbar__item` の非選択は hover で文字を ink まで持ち上げる。表のデータ行は `.sk-table tbody tr:hover` で無彩フィル。
- **フォーカスリング**: `:focus-visible` は少し外側から標準の 2px へ吸い込む(`@keyframes sk-focus-in`)。最終形は不変なので a11y に影響しない。
- **セグメント/タブのスライド追従(opt-in)**: コンテナに `.has-slide` を付け、選択ピルと同じ見た目の `.sk-slide-ind` を1つ入れて選択項目の位置・サイズへ動かす。位置決めはホスト側の責務(`.is-scrolled` と同様)。選択項目自身の面は消え、インジケータが唯一のピルになる。

```js
// 各 .sk-seg / .sk-tabbar にスライド追従インジケータを付与(抜粋)
function skSlide(container, itemSel) {
  const ind = Object.assign(document.createElement('span'), { className: 'sk-slide-ind' });
  container.classList.add('has-slide');
  container.prepend(ind);
  const place = (el, animate) => {
    if (!el || !el.offsetWidth) return;                 // 非表示中は測れないのでスキップ
    ind.style.transition = animate ? '' : 'none';
    ind.style.width = el.offsetWidth + 'px';
    ind.style.height = el.offsetHeight + 'px';
    ind.style.transform = `translate(${el.offsetLeft}px, ${el.offsetTop}px)`;
  };
  container.querySelectorAll(itemSel).forEach(it => it.addEventListener('click', () => place(it, true)));
  const cur = container.querySelector(itemSel + '.is-on') || container.querySelector(itemSel);
  place(cur, false);   // 端末フレーム内など初期非表示のものは IntersectionObserver で測り直す
}
```

> SwiftUI 版: `SukashiButtonStyle` は press(scale)に加え hover で 1px 浮く(`.onHover` / `accessibilityReduceMotion` でアニメ無効)。`SukashiSideNavItem` は非選択の hover に無彩フィル(`SukashiMenuItemStyle` は既に対応)。セグメントのスライドは Web 固有で、SwiftUI では選択に `matchedGeometryEffect` を使う想定。
...
**v2.2 部材も parity 済み**: プライマリー色切替(`.sukashiAccent(_:)`)、ボタン(`.sukashiPrimary/.sukashiDanger/.sukashiQuiet/.sukashiGlass`)、`SukashiNotice`、`SukashiField`(+ `.sukashiInvalid`)、`SukashiSteps`、`SukashiCode`/`SukashiKbd`、`SukashiMenuItemStyle`/`SukashiSideNavItem`、ink/fill/hairline/typography トークン、`prefers-contrast`(`colorSchemeContrast`)対応。

**追従は必須ルール**: CSS(リファレンス)を拡張したら、同じ変更で SwiftUI 版も更新して parity を保つ。詳細は `CLAUDE.md`。

## 今後の検討

ツールチップ、日付ピッカー、ページネーション、パンくず、Tailwind preset / React コンポーネントへの転写、SwiftUI 版の残部材(seg / tabbar / chip / switch / modal 等の View 化)、アイコンの追加(ラジオ・園芸・調理など個人アプリ向けセット)。

> v2.1 で対応済み: ダークモード(`.sk-dark`)、カテゴリ識別色(`.sk-cat-*`)、SwiftUI(ネイティブ)への転写。
> v2.2 で対応済み: 実アプリ部材(notice/danger/field/menu/sidenav/steps/code)、プライマリー色切替、コントラスト強化 — **CSS・SwiftUI 両実装**。
